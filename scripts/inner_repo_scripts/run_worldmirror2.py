...
# The script can be called from project root or from scripts/ directory
# Try multiple possible paths to find HY-World-2.0
possible_paths = [
    Path(__file__).resolve().parent.parent.parent / "models" / "external" / "hy-world-2.0",  # Called as uv run python scripts/...
    Path(__file__).resolve().parent.parent / "models" / "external" / "hy-world-2.0",  # Called from scripts/inner_repo_scripts/
    Path.cwd() / "models" / "external" / "hy-world-2.0",  # Assume project root is current dir
]

HYWORLD2_ROOT = None
for path in possible_paths:
    if path.exists() and (path / "hyworld2").exists():
        HYWORLD2_ROOT = path
        break

if HYWORLD2_ROOT is None:
    print("[worldmirror2] ERROR: Could not find HY-World-2.0 repository", file=sys.stderr)
    print(f"[worldmirror2] Searched paths:", file=sys.stderr)
    for path in possible_paths:
